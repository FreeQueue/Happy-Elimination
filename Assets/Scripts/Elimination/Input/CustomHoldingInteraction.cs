...
namespace Invertex.UnityInputExtensions.Interactions
{
    ...
    ///     Allowing a purely callback based approach to a button hold instead of polling it in an Update() loop or creating
    ///     specific logic for it
    ///     .started will be called when the 'pressPoint' threshold has been met and held for the 'duration' (unless 'Trigger
    ///     .started on Press Point' is checked).
    ///     .performed will continue to be called each frame after `.started` has triggered (or every amount of time set for
    ///     "Performed Interval")
	...
	[InitializeOnLoad, Preserve, DisplayName("Holding"), Serializable]
#endif
	public class CustomHoldingInteraction : IInputInteraction
	{

		#region Serialized Fields
		public float delayBetweenPerformed;
		public bool triggerStartedOnPressPoint;
		...
		private float pressPointOrDefault =>
			useDefaultSettingsPressPoint || pressPoint <= 0 ? InputSystem.settings.defaultButtonPressPoint : pressPoint;
		private float durationOrDefault =>
			useDefaultSettingsDuration || duration < 0 ? InputSystem.settings.defaultHoldTime : duration;

		#region IInputInteraction Members
		public void Process(ref InputInteractionContext context) {
			...
			if (ctx.phase != InputActionPhase.Performed && ctx.phase != InputActionPhase.Started) EnableInputHooks();
		}
		public void Reset() => Cancel(ref ctx);
		#endregion

		private void OnUpdate() {
			bool isActuated = ctx.ControlIsActuated(pressPointOrDefault);
			...
			if (phase == InputActionPhase.Canceled || phase == InputActionPhase.Disabled ||
				!ctx.action.actionMap.enabled || !isActuated) {
				Reset();
				return;
			}
	...
	{
		private static GUIContent pressPointWarning,
			holdTimeWarning,
			pressPointLabel,
			holdTimeLabel,
			startedTriggerOnPPLabel,
			startedTriggerOnPPToggleLabel,
			delayPerformedLabel;

		protected override void OnEnable() {
			delayPerformedLabel = new GUIContent("'Performed' interval (s)",
				$"Delay in seconds between each <b>.performed</b> call.{Environment.NewLine}" +
				"At the default value of 0, it will be every frame.");

			startedTriggerOnPPLabel = new GUIContent("Trigger 'Started' on Press Point",
				$"Trigger the <b>.started</b> event as soon as input actuated beyond \"Press Point\",{Environment.NewLine}" +
			...
			pressPointLabel = new GUIContent("Press Point",
				$"The minimum amount this input's actuation value must exceed to be considered \"held\".{Environment.NewLine}" +
			...
			holdTimeLabel = new GUIContent("Min Hold Time",
				$"The minimum amount of realtime seconds before the input is considered \"held\".{Environment.NewLine}" +
			...
			pressPointWarning
				= EditorGUIUtility.TrTextContent(
					"Using \"Default Button Press Point\" set in project-wide input settings.");
			holdTimeWarning
				= EditorGUIUtility.TrTextContent("Using \"Default Hold Time\" set in project-wide input settings.");
		}

		public override void OnGUI() {
			target.delayBetweenPerformed = EditorGUILayout.FloatField(delayPerformedLabel, target.delayBetweenPerformed,
				GUILayout.ExpandWidth(false));
			EditorGUILayout.BeginHorizontal();
			EditorGUILayout.LabelField(startedTriggerOnPPLabel, GUILayout.Width(205f));
			target.triggerStartedOnPressPoint = GUILayout.Toggle(target.triggerStartedOnPressPoint,
				startedTriggerOnPPToggleLabel, GUILayout.ExpandWidth(false));
			EditorGUILayout.EndHorizontal();

			DrawDisableIfDefault(ref target.pressPoint, ref target.useDefaultSettingsPressPoint, pressPointLabel,
				pressPointWarning);
			DrawDisableIfDefault(ref target.duration, ref target.useDefaultSettingsDuration, holdTimeLabel,
				holdTimeWarning, -Mathf.Epsilon);
		}

		private void DrawDisableIfDefault(
			ref float value, ref bool useDefault, GUIContent fieldName, GUIContent warningText, float compareOffset = 0f
		) {
			EditorGUILayout.BeginHorizontal();
			...
			GUIContent content =
				EditorGUIUtility.TrTextContent("Default",
					...
					$"configured globally in the input settings is used.{Environment.NewLine}" +

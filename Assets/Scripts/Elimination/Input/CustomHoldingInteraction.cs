...
namespace Invertex.UnityInputExtensions.Interactions
{
    ...
    [InitializeOnLoad]
#endif
    [Preserve, DisplayName("Holding"), Serializable]
    ...
    internal class CustomHoldInteractionEditor : InputParameterEditor<CustomHoldingInteraction>
    {
        ...
        {
            delayPerformedLabel = new GUIContent("'Performed' interval (s)", $"Delay in seconds between each <b>.performed</b> call.{Environment.NewLine}" +
            "At the default value of 0, it will be every frame.");
            
            startedTriggerOnPPLabel = new GUIContent("Trigger 'Started' on Press Point", $"Trigger the <b>.started</b> event as soon as input actuated beyond \"Press Point\",{Environment.NewLine}" +
            ...
            pressPointLabel = new GUIContent("Press Point", $"The minimum amount this input's actuation value must exceed to be considered \"held\".{Environment.NewLine}" +
            ...
            holdTimeLabel = new GUIContent("Min Hold Time", $"The minimum amount of realtime seconds before the input is considered \"held\".{Environment.NewLine}" +
